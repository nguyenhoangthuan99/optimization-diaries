...
description: "Same 17 GFLOP, from 0.3 to 93 GFLOP/s on one Zen 4 core — measured rung by rung with perf counters and cache-line diagrams."
---
...
We'll climb there one rung at a time, measuring each step (GFLOP/s *and*
...
doesn't. Keep that asymmetry in mind for the threading rungs later.
...
## Rung 0 — the loop everyone writes first
...
## Rung 1 — same flops, different spelling
...
## Rung 2 — tiling: make the working set fit
...
## Rung 3 — a humbling interlude: the compiler was already here

The next classical rung is *register blocking*: compute a small tile of C
...
it stays vectorizable — and measured… **38.8 GFLOP/s. A wash with rung 2.**

Two honest lessons from this rung. First: my *initial* version used the
other textbook formulation (4×4 tile, dot-product style, k innermost) and
ran **4× slower than rung 2** — a k-inner reduction with strided B kills
auto-vectorization, same as rung 0. Register blocking that breaks
...
misses per kflop fell from 631 (rung 0) to **7**, and the AVX-512 version
...
## Rung 5 — threads: many cores, one memory bus
...
destroyed rung 2. The fix is one contiguous row-panel per thread, so each
...
## Rung 6 — pinning: stop moving my caches
...
On a bare-metal multi-socket box this rung grows teeth: memory pages live on
...
- **The hierarchy is measurable, and worth measuring.** 1.3 ns → 4 → 9–42 →
  ~270: each rung of the ladder exists to keep the working set one level
...
- **The compiler gets you to the L2 rung, not past it.** Auto-vectorization
...
brags about GFLOP/s, you'll know which rung they're standing on.
